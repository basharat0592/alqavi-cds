...
export default function AddCustomerPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const msg = err?.response?.data?.detail || err?.response?.data?.email?.[0] || err?.response?.data?.password?.[0] || 'Failed to create customer.';
            showToast(msg, 'error');
        } finally {
            setSaving(false);
        }
    ...
    return (
        <div className="max-w-[1200px] mx-auto space-y-6 pb-12 font-sans px-3 sm:px-6 mt-6 relative z-0">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-xl font-black text-gray-900 tracking-tight">Add New Customer</h1>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Create a new customer account</p>
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                            <div>
                                ...
                                <input type="text" value={form.city} onChange={e => handle('city', e.target.value)}
                                    className={inputCls('city')} placeholder="New York" />
                            </div>
                            <div>
                                ...
                                <input type="text" value={form.country} onChange={e => handle('country', e.target.value)}
                                    className={inputCls('country')} placeholder="United States" />
                            </div>
                            <div>
                                ...
                                <input type="text" value={form.postal_code} onChange={e => handle('postal_code', e.target.value)}
                                    className={inputCls('postal_code')} placeholder="10001" />
                            </div>
                        </div>
                    </div>
