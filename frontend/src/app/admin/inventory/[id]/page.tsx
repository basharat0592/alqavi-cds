...
export default function StockViewPage() {
    ...
    if (loading) {
        return <div className="flex items-center justify-center py-40"><Loader2 className="w-8 h-8 animate-spin text-indigo-600" /></div>;
    }
    if (notFound || !stock) {
        return (
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-slate-200/70 rounded-xl divide-y md:divide-y-0 md:divide-x divide-slate-100 overflow-hidden bg-white">
                    ...
                    <div className="p-4 bg-slate-50">
                        ...
                        <p className="text-[18px] font-bold text-indigo-600 tabular-nums">{formatCurrency(qty * cost)}</p>
                    </div>
                </div>
