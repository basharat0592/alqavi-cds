...
const inputCls = (err?: boolean) => `w-full px-4 py-2.5 bg-white dark:bg-[#1B1C1E] border rounded-xl text-sm outline-none focus:border-[#1D4ED8] focus:ring-1 focus:ring-[#1D4ED8] transition-all placeholder:text-slate-400 text-slate-800 dark:text-slate-200 ${err ? 'border-red-600' : 'border-slate-200 dark:border-white/10'}`;
const selectCls = `w-full px-4 py-2.5 bg-white dark:bg-[#1B1C1E] border border-slate-200 dark:border-white/10 rounded-xl text-sm outline-none focus:border-[#1D4ED8] text-slate-600 dark:text-slate-300 cursor-pointer transition-all`;
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <RefreshCw className="h-8 w-8 animate-spin text-[#1D4ED8]" />
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto py-8 px-6 font-sans">
            <div className="mb-8">
                <Link 
                    ...
                    className="text-sm font-medium text-slate-500 hover:text-[#1D4ED8] transition-colors mb-4 flex items-center gap-1"
                >
                    <ChevronLeft className="h-4 w-4" /> Back to Warehouses
                </Link>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">Edit Warehouse</h1>
                <p className="text-sm text-slate-500">Update configuration for: {form.name}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button
                        type="submit"
                        disabled={saving}
                        className="flex items-center gap-2 px-8 py-2.5 bg-[#1D4ED8] text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-50"
                    >
