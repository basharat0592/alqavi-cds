                ...
                <div className="md:col-span-3 space-y-4">
                    <div className="bg-white border border-[#ddd] rounded-[4px] shadow-sm overflow-hidden">
                        <div className="bg-[#f7f8fa] border-b border-[#ddd] px-6 py-3 flex items-center justify-between">
                            ...
                            {currentMenu && (
                                <AmazonBtn onClick={() => setAddingItem(true)} variant="secondary" className="h-[28px] text-[12px]">
                        ...
                        <div className="p-0">
                            {!currentMenu ? (
                                ...
                                <div className="divide-y divide-[#eee]">
                                            ...
                                            {editingItem && editingItem.id === item.id ? (
                                                <div className="flex-1 flex items-center gap-3">
                                                    <input value={editingItem.title} onChange={e => setEditingItem({ ...editingItem, title: e.target.value } as NavigationItem)}
                                                        className={inputCls + " flex-1 font-bold"} placeholder="Label (e.g. Shop All)" />
                                                    <input value={editingItem.url} onChange={e => setEditingItem({ ...editingItem, url: e.target.value } as NavigationItem)}
                                                        className={inputCls + " flex-1 font-mono"} placeholder="URL (e.g. /shop)" />
                                                    <AmazonBtn onClick={saveEditItem} loading={saving} className="h-[31px]">Update</AmazonBtn>
                                                    <button onClick={() => setEditingItem(null)} className="p-2 text-slate-400 hover:text-[#111]"><X size={16} /></button>
                                                </div>
                                            ) : (
                                                <>
                                    ...
                                    {/* Add New Item */}
                                    {addingItem && (
                                        <div className="bg-[#fff9e6] px-6 py-5 border-t border-[#fbd38d]">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="pt-5 flex items-center gap-2">
                                                    <AmazonBtn onClick={addItem} loading={saving}>Add to Menu</AmazonBtn>
                                                    <button onClick={() => setAddingItem(false)} className="text-[13px] font-bold text-[#007185] hover:underline px-2">Cancel</button>
                                                </div>
                                            </div>
                                        </div>
                                    )}
