        ...
        <div className="max-w-[1400px] mx-auto px-4 py-8 bg-slate-50 dark:bg-[#070F14] min-h-screen font-sans">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1.5">
                            <span className="px-2 py-0.5 bg-blue-50 text-[#F59E0B] text-[9px] font-black uppercase tracking-widest rounded border border-blue-100">Inventory Module</span>
                ...
                <div className="flex items-center gap-3">
                    <button onClick={loadData} className="p-3 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-slate-400 hover:text-[#F59E0B] transition-all shadow-sm">
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
                <div className="relative flex-1 group">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-[#F59E0B] transition-all" />
                    <input 
                        value={search} onChange={e => setSearch(e.target.value)}
                        placeholder="Scan for specific asset identifier or name..." 
                        className="w-full pl-12 pr-4 py-3.5 bg-white dark:bg-[#1a252f] border border-slate-200 dark:border-white/10 rounded-2xl outline-none focus:border-[#F59E0B] focus:ring-4 focus:ring-[#F59E0B]/5 text-sm font-medium transition-all shadow-sm"
                    />
                </div>
                    ...
                    {(['all', 'low', 'out'] as const).map(f => (
                        <button key={f} onClick={() => setFilter(f)}
                            className={`px-5 py-2.5 text-[11px] font-black uppercase tracking-wider rounded-xl transition-all
                                ${filter === f ? 'bg-[#F59E0B] text-white shadow-lg' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}>
                                ...
                                return (
                                    <tr key={p.id} className="hover:bg-slate-50/30 dark:hover:bg-white/5 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 bg-slate-100 dark:bg-white/5 rounded-xl flex items-center justify-center font-black text-slate-400 text-xs border border-slate-200 dark:border-white/10 shadow-inner group-hover:bg-[#F59E0B] group-hover:text-white group-hover:border-[#F59E0B] transition-all">
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <div className="flex flex-col items-center">
                                                <p className={`text-sm font-black ${stock < 10 ? 'text-red-500' : 'text-[#F59E0B]'}`}>{stock}</p>
            ...
            <div className="mt-8 bg-[#F59E0B] p-8 rounded-2xl text-white flex flex-col md:flex-row justify-between items-center gap-8 shadow-2xl relative overflow-hidden group">
