...
const STATUS_MAP: Record<string, { label: string; cls: string }> = {
    ...
    'processing': { label: 'Pending', cls: 'bg-blue-100 text-[#F7CA00] border-blue-200' },
        ...
        <div className="max-w-[1500px] mx-auto px-4 py-8 font-sans">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-[#F7CA00] rounded-2xl flex items-center justify-center shadow-xl shadow-blue-500/20">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={loadInvoices} className="p-2.5 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1B1C1E] text-slate-500 hover:text-[#F7CA00] transition-all">
                        <RefreshCw className="h-4 w-4" />
                    </button>
                    <button 
                        onClick={() => router.push('/admin/sale')} 
                        className="bg-[#F7CA00] hover:bg-blue-700 text-white flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-blue-500/20 transition-all active:scale-95"
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {[
                    ...
                    <SectionCard key={i} className="p-4 flex items-center gap-4 border-l-4 border-l-[#F7CA00]">
            ...
            <div className="flex flex-col md:flex-row gap-3 mb-6">
                <div className="relative flex-1 group">
                    ...
                    <input 
                        ...
                        className="w-full bg-white dark:bg-[#1B1C1E] border border-slate-200 dark:border-white/10 rounded-xl pl-12 pr-4 py-3 text-sm font-medium outline-none focus:border-[#F7CA00] focus:ring-4 focus:ring-[#F7CA00]/10 transition-all placeholder:text-slate-400"
                    />
                </div>
                    ...
                    {['all', 'delivered', 'processing', 'pending'].map(s => (
                        <button 
                            ...
                            className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all
                                ${filterStatus === s ? 'bg-white dark:bg-[#1B1C1E] text-[#F7CA00] shadow-sm shadow-black/5 ring-1 ring-slate-200 dark:ring-white/10' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                                ...
                                filtered.map(inv => (
                                    <tr key={inv.id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors group">
                                        <td className="px-6 py-4">
                                            <span className="text-xs font-black text-[#F7CA00] uppercase px-2 py-1 bg-blue-50 dark:bg-blue-900/10 rounded">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-1.5 translate-x-2 group-hover:translate-x-0 transition-transform">
                                                <button 
                                                    ...
                                                    className="p-2 text-slate-400 hover:text-[#F7CA00] hover:bg-blue-50 dark:hover:bg-blue-900/10 rounded-xl transition-all" 
                                                    title="View Details"
                                                >
