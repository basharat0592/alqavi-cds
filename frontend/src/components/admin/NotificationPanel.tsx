            ...
            <div className="px-5 py-4 border-b border-gray-700 flex items-center justify-between bg-[#232F3E] rounded-t-2xl">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        {unread > 0 && (
                            <p className="text-[10px] font-bold text-[#EEAF1C] uppercase tracking-widest">
                                {unread} unread
                            </p>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    {unread > 0 && (
                        <button onClick={onMarkAllRead}
                            className="text-[10px] font-black text-[#EEAF1C] hover:underline uppercase tracking-wider px-2 py-1 hover:bg-white/10 rounded-lg transition-colors">
                            Mark all read
                        </button>
                    )}
            ...
            <div className="max-h-[420px] overflow-y-auto">
                {loading ? (
                    <div className="py-12 flex flex-col items-center justify-center gap-3">
                        <div className="w-8 h-8 border-3 border-[#EEAF1C]/20 border-t-[#EEAF1C] rounded-full animate-spin" />
                        ...
                        {activities.map(item => {
                            ...
                            return (
                                <Link key={item.id} href={item.href}
                                    ...
                                    className={`flex items-start gap-3.5 px-5 py-3.5 hover:bg-gray-50/80 dark:hover:bg-slate-800/50 transition-all group ${!item.read ? 'bg-[#EEAF1C]/5 dark:bg-[#EEAF1C]/10' : ''}`}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between gap-2">
                                            <p className={`text-sm font-bold leading-tight group-hover:text-[#EEAF1C] transition-colors ${!item.read ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-slate-300'}`}>
                                                {item.title}
                                            </p>
                                            {!item.read && <span className="w-2 h-2 bg-[#EEAF1C] rounded-full flex-shrink-0 mt-1.5" />}
                                        </div>
