...
const PRIMARY_BTN = "bg-[#1D4ED8] hover:bg-[#1e40af] text-white font-bold rounded-lg shadow-sm text-[11px] uppercase tracking-widest py-2 px-4 transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50";
        ...
        <div className="max-w-[1400px] mx-auto pb-20 font-sans px-4 mt-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-8 pb-4 border-b border-slate-200 dark:border-white/10">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#1D4ED8] rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5 text-[9px] font-black text-[#1D4ED8] uppercase tracking-widest pl-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-12 space-y-12">
                        ...
                        {alerts.length === 0 ? (
                            <SectionCard className="p-12 text-center">
                                <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900/10 rounded-2xl flex items-center justify-center mx-auto mb-6">
                                    <CheckCircle2 className="h-8 w-8 text-[#1D4ED8]" />
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                {alerts.map(a => (
                                    <SectionCard key={a.id} className="group hover:border-[#1D4ED8]/30 transition-all">
                                        <div className="p-5 flex flex-col h-full">
                                            ...
                                            <div className="flex-1">
                                                <Link href={a.href || '#'} className="block group/link">
                                                    <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight truncate group-hover/link:text-[#1D4ED8] transition-colors">{a.product}</h3>
                                                </Link>
                                            ...
                                            <Link href={`/admin/purchases/add?product_id=${a.productId}&product_name=${encodeURIComponent(a.product)}&supplier_name=${encodeURIComponent(a.supplierName)}&quantity=0`}
                                                className="w-full flex items-center justify-center gap-2 py-2.5 bg-slate-900 hover:bg-slate-800 text-white dark:bg-white/10 dark:hover:bg-[#1D4ED8] rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
                    ...
                    <section className="space-y-6">
                        <div className="flex items-center justify-between border-l-4 border-[#1D4ED8] pl-5 py-0.5">
                            <div>
                                <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                                    <ClipboardList className="h-4 w-4 text-[#1D4ED8]" /> Operational Lifecycle Hub
                                </h2>
                            ...
                            <div className="space-y-4">
                                {activities.map((act) => (
                                    <div key={act.id} className="relative pl-14 group animate-in slide-in-from-left-4 duration-500">
                                        ...
                                        <SectionCard className="p-5 hover:border-[#1D4ED8]/30 transition-all group-hover:bg-slate-50/10">
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
                                                ...
                                                <div className="text-[9px] font-black text-slate-400 flex items-center gap-1.5 uppercase tracking-[0.2em]">
                                                    <Clock size={10} className="text-[#1D4ED8] group-hover:animate-spin" /> {act.time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                </div>
                                            </div>
