...
export default function SalesStatementsPage() {
    ...
    const sel = useTableSelection(mockData, (s) => s.id);
    ...
    return (
        <div>
            <div className="hidden print:block mb-4">
                <InvoiceHeader docTitle="Sales Report" />
            </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {mockData.map((st, i) => (
                            <tr key={i} className="hover:bg-slate-50 transition-colors group text-[13px]">
                                <RowCheckboxTd sel={sel} id={st.id} />
                                ...
                                <td className="px-3 sm:px-6 py-3 sm:py-4 text-center">
                                    <button className="p-1.5 border border-slate-200 rounded-lg bg-white hover:bg-slate-50 text-slate-500 shadow-sm transition-colors print:hidden">
                                        <Download size={14} />
                                    </button>
                                </td>
                            </tr>
                ...
                <BulkBar
                    ...
                    onExport={() => exportToCSV(
                        sel.selectedItems.map((st: any) => ({
                            statement_number: st.id,
                            date: st.date,
                            customer: st.name,
                            orders: st.count,
                            total_sales: st.d,
                            total_paid: st.c,
                            remaining_balance: st.b,
                        })),
                        'sales-statements.csv',
                    )}
                />
            </div>
