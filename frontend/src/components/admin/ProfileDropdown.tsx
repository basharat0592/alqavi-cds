...
export default function ProfileDropdown({
    ...
    const inputCls = "w-full px-3.5 py-2.5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-4 focus:ring-[#F59E0B]/10 focus:border-[#F59E0B] text-sm font-medium text-slate-700 dark:text-white transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600";

    const MENU_ITEMS = [
        { icon: User, label: 'View Profile', href: '/admin/settings', color: 'text-[#F59E0B]', bg: 'bg-[#F59E0B]/10' },
    ...
    return (
        <div className="absolute top-full right-0 mt-2.5 w-[280px] bg-white/95 dark:bg-[#232F3E]/95 backdrop-blur-2xl rounded-2xl border border-slate-200 dark:border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden z-50 animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="bg-gradient-to-br from-[#1a252f] via-[#232F3E] to-[#F59E0B] px-4 pt-6 pb-5 relative overflow-hidden">
                {/* Decorative element */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-white/5 rounded-full -mr-12 -mt-12 blur-2xl" />
                
                <div className="flex items-center gap-3 relative z-10">
                    <div className="w-11 h-11 bg-[#F59E0B] rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden shadow-2xl border-2 border-white/20 group hover:rotate-6 transition-transform duration-500">
                        {user.avatar ? (
            ...
            <div className="py-2.5 px-1.5">
                {MENU_ITEMS.map(item => (
                    <Link key={item.label} href={item.href} onClick={onClose}
                        ...
                        <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 group-hover:text-[#F59E0B] dark:group-hover:text-[#F59E0B] flex-1 text-left uppercase tracking-wide transition-colors">{item.label}</span>
                        <ChevronRight className="h-3.5 w-3.5 text-slate-300 dark:text-slate-700 group-hover:text-[#F59E0B] group-hover:translate-x-1 transition-all" strokeWidth={3} />
                    </Link>
                ))}
