...
const fieldCls = `w-full px-3 py-2 text-sm bg-white dark:bg-[#1a252f] border border-slate-200 dark:border-white/10 rounded-lg outline-none focus:border-[#F59E0B] focus:ring-2 focus:ring-[#F59E0B]/10 transition-all placeholder:text-slate-400 text-slate-800 dark:text-slate-200`;
                    ...
                    <div className="p-8 text-center">
                        ...
                        <div className="bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-lg p-4 text-left mb-6">
                            <div className="flex justify-between items-center mb-3 pb-3 border-b border-slate-200 dark:border-white/10">
                                ...
                                <span className="text-2xl font-bold text-[#F59E0B]">{formatCurrency(successOrder.total_amount)}</span>
                            </div>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={resetPOS} className="flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium text-white bg-[#F59E0B] hover:bg-blue-700 rounded-lg shadow-sm transition-colors">
    ...
    return (
        <div className="max-w-[1700px] mx-auto px-4 py-4 font-sans">
            ...
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-200 dark:border-white/10">
                ...
                <button onClick={() => loadData(true)} disabled={isRefreshing} className="p-2 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-500 hover:text-[#F59E0B] transition-all disabled:opacity-50">
                    <RefreshCw className={`h-4 w-4 ${isRefreshing ? 'animate-spin text-[#F59E0B]' : ''}`} />
                </button>
            </div>
                ...
                <div className="lg:col-span-8 space-y-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        <div className="flex items-center bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg overflow-hidden">
                            <button onClick={() => setViewMode('grid')} className={`px-3 py-2 transition-colors ${viewMode === 'grid' ? 'bg-[#F59E0B] text-white' : 'text-slate-400 hover:text-slate-600'}`}>
                                <LayoutGrid className="h-4 w-4" />
                            </button>
                            <button onClick={() => setViewMode('list')} className={`px-3 py-2 transition-colors ${viewMode === 'list' ? 'bg-[#F59E0B] text-white' : 'text-slate-400 hover:text-slate-600'}`}>
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`group relative flex flex-col bg-white dark:bg-[#1a252f] border rounded-xl transition-all text-left overflow-hidden shadow-sm
                                                ${isOutOfStock ? 'opacity-40 grayscale cursor-not-allowed' : 'hover:border-[#F59E0B]/50 hover:shadow-md hover:-translate-y-0.5'}
                                                ${inCart > 0 ? 'border-[#F59E0B] ring-2 ring-[#F59E0B]/10' : 'border-slate-200 dark:border-white/10'}`}
                                        >
                                            <div className="aspect-square flex items-center justify-center bg-slate-50 dark:bg-white/5 relative p-4">
                                                ...
                                                {inCart > 0 && (
                                                    <div className="absolute top-2 right-2 w-6 h-6 bg-[#F59E0B] text-white rounded-full flex items-center justify-center text-xs font-bold shadow-md">
                                                        {inCart}
                                                    </div>
                                                )}
                                            </div>
                                            <div className="p-3 flex-1 flex flex-col">
                                                ...
                                                <div className="flex justify-between items-end">
                                                    <span className="text-sm font-bold text-[#F59E0B]">{formatCurrency(p.price)}</span>
                                        ...
                                        {paginatedProducts.map(p => {
                                            ...
                                            return (
                                                <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.01]">
                                                    ...
                                                    <td className="px-4 py-3 text-right font-semibold text-[#F59E0B]">{formatCurrency(p.price)}</td>
                                                    ...
                                                    <td className="px-4 py-3 text-center">
                                                        <button onClick={() => addToCart(p)} disabled={stock <= 0} className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-500 hover:bg-[#F59E0B] hover:text-white transition-colors disabled:opacity-30">
                    ...
                    <div className="bg-white dark:bg-[#1a252f] border border-slate-200 dark:border-white/10 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div className="p-4 border-b border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-white/5">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                {totalItems > 0 && <span className="ml-auto text-xs font-semibold px-2 py-0.5 bg-[#F59E0B]/10 text-[#F59E0B] rounded-full">{totalItems} item{totalItems !== 1 ? 's' : ''}</span>}
                            </div>
                            <div className="space-y-2">
                                <select
                                    value={customerId || ''}
                                    onChange={e => setCustomerId(e.target.value || null)}
                                    className="w-full px-3 py-2 text-sm bg-white dark:bg-[#1a252f] border border-slate-200 dark:border-white/10 rounded-lg outline-none focus:border-[#F59E0B] text-slate-700 dark:text-slate-300 cursor-pointer"
                                >
                                ...
                                {!customerId && (
                                    <input
                                        ...
                                        className="w-full px-3 py-2 text-sm bg-white dark:bg-[#1a252f] border border-slate-200 dark:border-white/10 rounded-lg outline-none focus:border-[#F59E0B] placeholder:text-slate-400"
                                    />
                                )}
                            </div>
                        </div>
                                    ...
                                    <div key={item.product.id} className="flex gap-3 p-3 items-start">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="text-sm font-semibold text-[#F59E0B]">{formatCurrency(String(parseFloat(item.product.price || 0) * (Number(item.quantity) || 0)))}</span>
                                            </div>
                                        </div>
                        ...
                        <div className="p-4 bg-slate-900 dark:bg-slate-950 border-t border-white/10">
                            ...
                            <button
                                ...
                                className="w-full py-2.5 bg-[#F59E0B] text-white text-sm font-semibold rounded-lg hover:bg-blue-600 transition-colors flex items-center justify-center gap-2 disabled:opacity-30 shadow-lg shadow-blue-500/30"
                            >
                    ...
                    <div className="bg-white dark:bg-[#1a252f] border border-slate-200 dark:border-white/10 rounded-xl max-w-sm w-full shadow-xl relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden">
                        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-white/10">
                            <div className="flex items-center gap-2">
                                <CardIcon className="h-4 w-4 text-[#F59E0B]" />
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <button
                                ...
                                className="w-full py-2.5 bg-[#F59E0B] text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-sm"
                            >
                                Confirm Card
                            </button>
                        </div>
                    ...
                    <div className="bg-white dark:bg-[#1a252f] border border-slate-200 dark:border-white/10 rounded-xl max-w-sm w-full shadow-xl relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden">
                        <div className="p-6 text-center">
                            <div className="w-14 h-14 bg-[#F59E0B]/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
                                {paymentMethod === 'cash' ? <Banknote className="h-7 w-7 text-[#F59E0B]" /> : <CardIcon className="h-7 w-7 text-[#F59E0B]" />}
                            </div>
                            ...
                            <div className="bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-lg p-4 my-4">
                                ...
                                <p className="text-2xl font-bold text-[#F59E0B]">{formatCurrency(String(cartTotal))}</p>
                            </div>
                            <div className="flex flex-col gap-2">
                                <button onClick={() => { setShowConfirmModal(false); handleCompleteSale(true); }} className="w-full py-2.5 bg-[#F59E0B] text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-sm">
                                    Confirm & Complete
                                </button>
