...
/* ─────────────────────────────────────────────────────────────────────────────
   PURE AMAZON RETAIL DESIGN SYSTEM - PURCHASE INVOICE
   ───────────────────────────────────────────────────────────────────────────── */
...
export default function PurchaseInvoicePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!purchase) return <div className="p-20 text-center font-bold">Purchase record not found.</div>;
            ...
            <div className="max-w-[850px] mx-auto pt-8 px-4 print:hidden">
                <div className="flex items-center justify-between py-4 border-b border-[#eee]">
                    <div className="flex items-center gap-1 text-[11px] text-[#565959] uppercase tracking-wider font-bold">
                        ...
                        <span className="cursor-pointer hover:text-[#c45500]" onClick={() => router.push('/admin/purchases')}>Purchases</span>
                        <ChevronRight size={10} />
                        <span className="text-[#c45500]">PO Invoice</span>
                    </div>
            ...
            <div className="max-w-[850px] mx-auto bg-white p-8 print:border-none print:shadow-none print:p-0">
                ...
                <div className="flex justify-between items-start mb-12">
                    ...
                    <div className="w-1/3 text-right">
                        <h2 className="text-[20px] font-black uppercase tracking-tighter text-[#111]">Purchase Order</h2>
                        ...
                        <p className="text-[14px] text-[#111] font-bold mt-4 tracking-tight">PO No: {purchase.purchase_number}</p>
                ...
                <div className="grid grid-cols-4 gap-8 mb-16 px-1">
                    <div className="col-span-2">
                        <h3 className="text-[10px] font-black text-[#bbb] uppercase mb-4 tracking-widest border-b border-[#eee] pb-1">Supplier Information</h3>
                        <p className="text-[18px] font-black text-black leading-none">{purchase.supplier_name}</p>
                        {purchase.supplier_company && <p className="text-[13px] text-[#565959] mt-2 font-bold">{purchase.supplier_company}</p>}
                        <p className="text-[13px] font-medium text-black mt-1">{purchase.supplier_phone || 'N/A'}</p>
                        <p className="text-[11px] text-gray-400 mt-2 w-64 leading-relaxed italic">{purchase.supplier_address || 'Verified Wholesale Partner'}</p>
                    </div>
                    <div>
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <span className="inline-block px-3 py-1 bg-black text-white text-[10px] font-black uppercase tracking-widest rounded-sm">
                    ...
                    <div className="text-right">
                        <h3 className="text-[10px] font-black text-[#bbb] uppercase mb-4 tracking-widest border-b border-[#eee] pb-1">Reception</h3>
                        <div className="space-y-4">
                            <div>
                                <p className="text-[9px] text-[#565959] font-bold uppercase tracking-widest mb-1">Inventory Sync</p>
                                {purchase.is_inventory_synced ? (
                                    ...
                                    <div className="text-amber-600 font-bold text-[10px] uppercase">Pending Sync</div>
                                )}
                            </div>
                            {purchase.reference_number && (
                                <div>
                                    <p className="text-[9px] text-[#565959] font-bold uppercase tracking-widest mb-1">Ext Ref</p>
                                    <p className="text-[12px] font-bold text-black">{purchase.reference_number}</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="text-[14px]">
                                ...
                                return (
                                    <tr key={i} className="hover:bg-gray-50/50 border-b border-[#eee]">
                                        ...
                                        <td className="py-4 px-3 font-bold text-[#111]">
                                            {item.product_name}
                                            <div className="text-[10px] font-medium text-gray-400 mt-0.5">Packaging: {item.packaging_type?.toLowerCase()}</div>
                                        </td>
                                        <td className="py-4 px-3 text-center font-bold text-emerald-600">{qty}</td>
                            ...
                            {purchase.shipping_cost > 0 && (
                                <tr>
                                    <td colSpan={3}></td>
                                    <td className="py-2 text-right text-[12px] font-bold text-gray-500 uppercase">Shipping Fees</td>
                                    <td className="py-2 text-right font-black text-[#111]">{formatCurrency(purchase.shipping_cost)}</td>
                                </tr>
                            )}
                            <tr className="bg-gray-50/50">
                                ...
                                <td className="py-4 px-3 text-right text-[14px] font-black uppercase tracking-wider text-black">Total Amount</td>
                ...
                {/* Terms Area */}
                <div className="mt-12 mb-6 px-1 border-l-4 border-black pl-5">
                    <p className="text-[12px] font-bold text-black uppercase mb-1">Company Settlement Policy</p>
                    <p className="text-[10px] leading-relaxed text-gray-500 max-w-lg">
                ...
                <div className="mt-16 pt-12 border-t-2 border-dashed border-black">
                    <div className="flex justify-between items-start gap-32">
                        <div className="flex-1 space-y-3 text-center">
                            <p className="text-[11px] font-bold text-gray-400 uppercase">Supplier Confirmation</p>
                            <div className="w-full border-b border-black pt-10"></div>
                            <p className="text-[12px] font-black uppercase tracking-widest text-black pt-2">Authorized Sign/Stamp</p>
                        </div>
                        <div className="flex-1 space-y-3 text-center">
                            <p className="text-[11px] font-bold text-gray-400 uppercase">Company Audit Verification</p>
                            <div className="w-full border-b border-black pt-10"></div>
                            <p className="text-[12px] font-black uppercase tracking-widest text-black pt-2">Gilgit Finance Desk</p>
                        </div>
                    </div>
