                ...
                {order ? (
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in duration-700">
                            ...
                            <Card className="overflow-hidden">
                                <div className="bg-slate-50/60 px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-[14px] font-bold text-[#B4780B]">{formatDate(order.updated_at)}</p>
                                    </div>
                                </div>
                                <div className="p-10 pb-16">
                                    <div className="relative">
                                        ...
                                        {currentStepIndex >= 0 && order.status !== 'CANCELLED' && (
                                            <div
                                                className="absolute top-1/2 left-0 h-1 bg-[#F59E0B] -translate-y-1/2 rounded-full transition-all duration-1000 ease-out"
                                                ...
                                                return (
                                                    <div key={idx} className="flex flex-col items-center group">
                                                        <div className={`w-8 h-8 rounded-full border flex items-center justify-center bg-white z-10 transition-all duration-500
                                                            ${isCompleted ? 'border-[#F59E0B] text-[#B4780B] ring-4 ring-[#F59E0B]/10' : 'border-slate-200 text-slate-400'}`}>
                                                            {isCompleted ? <CheckCircle2 size={16} fill="currentColor" className="text-white bg-[#F59E0B] rounded-full" /> : <StepIcon size={14} />}
                                                        </div>
                                                        <div className="absolute top-10 flex flex-col items-center">
                                                            <p className={`text-[10px] font-bold uppercase tracking-tight whitespace-nowrap ${isCompleted ? 'text-[#B4780B]' : 'text-slate-400'}`}>
                                                                {step.label}
                                                            </p>
                                                            {isCurrent && <span className="w-1.5 h-1.5 bg-[#F59E0B] rounded-full mt-1.5 animate-ping" />}
                                                        </div>
                                                    </div>
                                                );
                                            })}
                                    ...
                                    {order.items?.map((item: any) => (
                                        <div key={item.id} className="p-5 flex items-center justify-between hover:bg-slate-50 transition-colors">
                                            <div className="flex gap-4 items-center">
                                                ...
                                                <div>
                                                    <p className="text-[13px] font-bold text-slate-800 hover:text-[#92600A] cursor-pointer">{item.product_name}</p>
                        ...
                        <div className="lg:col-span-4 space-y-6">
                            ...
                            <Card className="overflow-hidden">
                                ...
                                <div className="p-5 space-y-5 text-left">
                                    <div>
                                        ...
                                        <p className="text-[14px] font-bold text-slate-800 hover:text-[#92600A] cursor-pointer">{order.supplier_name || 'Partner'}</p>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-1.5 mt-2 text-[#B4780B] hover:text-[#92600A] cursor-pointer text-[12px] font-bold">
                            ...
                            <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/15 rounded-2xl p-4 flex gap-3 text-left">
                                <ShieldCheck className="h-5 w-5 text-[#B4780B] shrink-0" />
                                <div>
                                    <p className="text-[12px] font-bold text-[#B4780B]">Audit Certified</p>
                                    <p className="text-[11px] text-[#B4780B]/80 mt-1 leading-relaxed">This record is end-to-end encrypted and synced with financial ledgers.</p>
                                </div>
                            </div>
                    ...
                    <Card className="py-32 flex flex-col items-center justify-center animate-in zoom-in-95 duration-500">
                        <div className="relative mb-6">
                            ...
                            <Search size={32} className="absolute -bottom-2 -right-2 text-[#B4780B] bg-white rounded-full p-2 shadow-sm border border-slate-200/70" />
                        </div>
