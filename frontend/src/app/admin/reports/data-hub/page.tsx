...
const ExportOption = ({ title, desc, icon: Icon, onClick, loading }: { 
    ...
    <div className="p-8 flex flex-col justify-between h-full bg-white dark:bg-[#1a252f] rounded-xl border border-slate-100 dark:border-white/5 hover:border-[#F59E0B] transition-all hover:shadow-lg">
        <div>
            <div className={`w-12 h-12 rounded-xl bg-blue-50 dark:bg-[#F59E0B]/10 flex items-center justify-center mb-6`}>
        ...
        <button 
            ...
            className={`w-full py-2.5 flex items-center justify-center gap-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all
                ${loading 
                    ...
                    : 'bg-[#F59E0B] text-white hover:bg-blue-700 shadow-md active:scale-[0.98]'}`}
        >
        ...
        <div className="max-w-[1400px] mx-auto px-4 py-8 bg-slate-50 dark:bg-[#070F14] min-h-screen font-sans">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 pb-4 border-b border-slate-200 dark:border-white/10">
                <div className="flex items-center gap-4">
                    <div className="p-2.5 bg-[#F59E0B] rounded-xl shadow-lg shadow-blue-500/20">
                ...
                <div className="p-3 bg-blue-50 dark:bg-[#F59E0B]/10 rounded-lg border border-blue-100 dark:border-white/5 flex items-center gap-3">
                    <AlertCircle className="h-4 w-4 text-[#F59E0B]" />
                    <p className="text-[10px] font-bold text-[#F59E0B] uppercase tracking-tight">Enterprise Export Enabled</p>
                </div>
            </div>
            ...
            <SectionCard className="p-8 mb-10 bg-white">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-end">
                    <div className="space-y-2">
                        ...
                        <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm bg-slate-50 outline-none focus:border-[#F59E0B] focus:bg-white transition-all shadow-inner" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-400 uppercase ml-2 tracking-widest">Chronos End (To)</label>
                        <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm bg-slate-50 outline-none focus:border-[#F59E0B] focus:bg-white transition-all shadow-inner" />
                    </div>
            ...
            <div className="mt-16 bg-[#131921] p-10 rounded-[2rem] text-white flex flex-col md:flex-row justify-between items-center gap-8 shadow-2xl">
                ...
                <div className="p-6 border-4 border-white/5 rounded-2xl rotate-3 transition-transform hover:rotate-0 duration-700 bg-white/5 backdrop-blur-md shadow-xl">
                    <Database className="h-10 w-10 text-[#F59E0B]" />
                </div>
            </div>
        </div>
    );
