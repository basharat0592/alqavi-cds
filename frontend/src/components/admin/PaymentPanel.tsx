        ...
        <div className={`space-y-4 ${className}`}>
            ...
            <div className="space-y-1">
                ...
                <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${remaining <= 0 ? 'bg-emerald-500' : 'bg-indigo-500'}`} style={{ width: `${pct}%` }} />
                </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 rounded-xl border border-slate-200 px-3 py-2.5">
                ...
                <input
                    ...
                    className="ml-auto h-8 rounded-lg border border-slate-200 px-2.5 text-[12.5px] text-slate-800 outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 disabled:bg-slate-50 disabled:text-slate-400"
                />
            </div>
                    ...
                    <div className="divide-y divide-slate-50 max-h-56 overflow-y-auto">
                        {items.map((it, idx) => (
                            <div key={it.id} className="flex items-center gap-3 px-3 py-2">
                                ...
                                {it.slip_url && (
                                    <a href={it.slip_url} target="_blank" rel="noreferrer" className="p-1 text-slate-400 hover:text-indigo-600" title="View slip">
            ...
            {!readOnly && remaining > 0 && (
                <div className="rounded-xl border border-indigo-100 bg-indigo-50/30 p-3 space-y-2.5">
                    <div className="flex items-center gap-2">
                        <Plus className="w-3.5 h-3.5 text-indigo-500" />
                        <span className="text-[11px] font-bold uppercase tracking-wider text-indigo-600">Record a Payment</span>
                        <button
                            type="button"
                            onClick={() => setAmount(String(remaining))}
                            className="ml-auto text-[10.5px] font-bold text-indigo-600 hover:underline"
                        >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full h-9 px-2.5 rounded-lg border border-slate-200 text-[13px] outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 tabular-nums"
                            />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-slate-500 mb-1">Method</label>
                            <select value={method} onChange={e => setMethod(e.target.value)}
                                className="w-full h-9 px-2 rounded-lg border border-slate-200 text-[13px] bg-white outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" value={paidAt} onChange={e => setPaidAt(e.target.value)}
                                className="w-full h-9 px-2.5 rounded-lg border border-slate-200 text-[12.5px] outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-slate-500 mb-1">Reference / Cheque #</label>
                            <input value={reference} onChange={e => setReference(e.target.value)} placeholder="Optional"
                                className="w-full h-9 px-2.5 rounded-lg border border-slate-200 text-[13px] outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10" />
                        </div>
                    </div>
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <button
                            ...
                            className="ml-auto inline-flex items-center gap-1.5 h-9 px-4 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-[12.5px] font-bold disabled:opacity-50 transition-colors"
                        >
